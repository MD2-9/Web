...
### 2.1 交互规范
1. **收起状态 (80px)**：仅显示图标，顶部展示圆形头像，底部展示暗色切换与主题配置等全局按钮。
2. **Hover 展开 (256px)**：鼠标移入导轨区域平滑展开，文字标签以 `opacity: 1; transition: opacity 0.3s;` 渐显；鼠标移出或点击空白处收回。
3. **二级抽屉面板 (Secondary Overlay Panels)**：
